...
async function windowData(symbol,tf,from,to){
 ...
 if(Array.isArray(market))return market.filter(b=>b[0]>=from&&b[0]<to);
 const chunks=market.chunks.filter(c=>c.to>=from&&c.from<to),keys=new Set(chunks.map(c=>c.key));
 const rows=(await Promise.all(chunks.map(c=>load(c.key)))).flat().filter(b=>b[0]>=from&&b[0]<to);
...
async function history(symbol,tf){
 ...
 const data=await load(symbol+'_'+tf),step={'15m':900,'1h':3600,'4h':14400,'1d':86400}[tf];
 return {start:data[0][0],step,values:Float64Array.from(data,b=>b[4]==null?NaN:b[4])};
}
